import { ToolType } from "@/types/ring";
import { cn } from "@/lib/utils";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";

interface ToolRailProps {
  activeTool: ToolType | null;
  onSelectTool: (tool: ToolType) => void;
  onApplyTool: (tool: ToolType) => void;
}

const TOOLS: { id: ToolType; name: string; icon: string; tip: string; sculpt?: boolean }[] = [
  { id: "carve", name: "Carve", icon: "⚒", tip: "Subtract material — reduces thickness" },
  { id: "smooth", name: "Smooth", icon: "✋", tip: "Polish & soften edges" },
  { id: "bevel", name: "Bevel", icon: "◇", tip: "Round off hard edges" },
  { id: "groove", name: "Groove", icon: "═", tip: "Cut channels into the band" },
  { id: "flatten", name: "Flatten", icon: "▬", tip: "Create flat profile" },
  { id: "measure", name: "Measure", icon: "📐", tip: "View exact dimensions" },
  { id: "stamp", name: "Stamp", icon: "⊙", tip: "Place dent/scratch marks on the surface (wax mode)" },
  { id: "push", name: "Push", icon: "👆", tip: "Push wax inward — displace surface at click point (wax mode)", sculpt: true },
  { id: "sculpt-carve", name: "Sculpt", icon: "🔪", tip: "Carve deep channels into wax surface (wax mode)", sculpt: true },
  { id: "sculpt-smooth", name: "Polish", icon: "🫧", tip: "Smooth and blend wax surface marks (wax mode)", sculpt: true },
];

export default function ToolRail({ activeTool, onSelectTool, onApplyTool }: ToolRailProps) {
  const paramTools = TOOLS.filter((t) => !t.sculpt);
  const sculptTools = TOOLS.filter((t) => t.sculpt);

  return (
    <div className="flex flex-col gap-1 p-2 bg-card rounded-lg border border-border">
      <span className="text-[10px] uppercase tracking-widest text-muted-foreground px-2 py-1 font-display">
        Tools
      </span>
      {paramTools.map((tool) => {
        const isActive = activeTool === tool.id;
        return (
          <Tooltip key={tool.id}>
            <TooltipTrigger asChild>
              <button
                onClick={() => {
                  onSelectTool(tool.id);
                  if (tool.id !== "measure" && tool.id !== "stamp") onApplyTool(tool.id);
                }}
                className={cn(
                  "flex flex-col items-center gap-0.5 px-3 py-2.5 rounded-md transition-all text-sm",
                  isActive
                    ? "bg-primary/20 text-primary ember-glow"
                    : "text-muted-foreground hover:bg-secondary hover:text-foreground"
                )}
              >
                <span className="text-lg">{tool.icon}</span>
                <span className="text-[10px] font-medium">{tool.name}</span>
              </button>
            </TooltipTrigger>
            <TooltipContent side="right" className="bg-popover text-popover-foreground border-border">
              {tool.tip}
            </TooltipContent>
          </Tooltip>
        );
      })}

      {/* Sculpting tools divider */}
      <div className="mx-2 my-1 border-t border-border/50" />
      <span className="text-[8px] uppercase tracking-widest text-muted-foreground/60 px-2 font-display">
        Sculpt
      </span>
      {sculptTools.map((tool) => {
        const isActive = activeTool === tool.id;
        return (
          <Tooltip key={tool.id}>
            <TooltipTrigger asChild>
              <button
                onClick={() => onSelectTool(tool.id)}
                className={cn(
                  "flex flex-col items-center gap-0.5 px-3 py-2.5 rounded-md transition-all text-sm",
                  isActive
                    ? "bg-primary/20 text-primary ember-glow"
                    : "text-muted-foreground hover:bg-secondary hover:text-foreground"
                )}
              >
                <span className="text-lg">{tool.icon}</span>
                <span className="text-[10px] font-medium">{tool.name}</span>
              </button>
            </TooltipTrigger>
            <TooltipContent side="right" className="bg-popover text-popover-foreground border-border">
              {tool.tip}
            </TooltipContent>
          </Tooltip>
        );
      })}
    </div>
  );
}
