...
function ProceduralRingMesh({ params, viewMode, metalPreset, finishPreset, activeTool, onAddWaxMark, stampSettings, lunarTexture, wearPreview = 0, polishPreview = 0, detailBoost = 0, thicknessHeatmap = false, onGenProgress }: RingMeshProps & { onGenProgress?: (p: GenerationProgress | null) => void }) {
  ...
  const detailAmount = detailBoost;
    ...
    const initialCamPos: [number, number, number] = insp
      ...
      return (
        ...
        {!isMobile && detailBoost > 0 && (
          <div className="absolute top-3 right-3 z-20 pointer-events-none animate-in fade-in slide-in-from-right-2 duration-200">
            <div className="flex flex-col gap-0.5 px-2.5 py-1.5 bg-secondary/20 backdrop-blur-sm border border-secondary/40 rounded-lg">
              <div className="flex items-center gap-1.5">
                <svg className="w-3.5 h-3.5 text-secondary-foreground/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" />
                </svg>
                <span className="text-[10px] font-medium text-secondary-foreground/80">
        ...
        <Canvas
          ...
          frameloop={turntableSpeed > 0 || (surfaceProgress && surfaceProgress.stage !== "done") ? "always" : "demand"}
          gl={{
