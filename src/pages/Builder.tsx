...
import { Move, RefreshCw, Wand2 } from "lucide-react";
import ViewportControls from "@/components/builder/ViewportControls";
...
function BuilderInner() {
  ...
  // Read-only destructured aliases for passing as props
  const {
    cameraPreset, showMeasurements, cutawayMode, cutawayOffset,
    lighting, showcaseMode, inspectionMode, ringPosition, ringRotation,
    showPrinterBed, rotationLocked, scaleReference, wearPreview,
    polishPreview, detailBoost, thicknessHeatmap, turntableSpeed,
    bgPreset, loupeActive, loupeZoom,
  } = vp;
    ...
    <div className="h-screen flex flex-col bg-forge-dark overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0 relative">
        ...
        <div ref={viewportContainerRef} className="flex-1 p-0 sm:p-1 relative builder-viewport-bg">
          ...
          {/* Tolerance alert bar — shows critical/warning count from castability */}
          {!isMobile && castabilityReport.level !== "good" && (
            <div className={cn(
              "absolute bottom-3 left-1/2 -translate-x-1/2 z-20 pointer-events-auto animate-in fade-in slide-in-from-bottom-2 duration-300",
            )}>
              <div className={cn(
                "flex items-center gap-2 px-3 py-1.5 rounded-full backdrop-blur-xl border shadow-lg text-[10px] font-medium",
                castabilityReport.level === "risk"
                ...
                <span>
                  {(() => {
                    const risks = castabilityReport.checks.filter(c => c.status === "risk");
                    const warns = castabilityReport.checks.filter(c => c.status === "warn");
                    const parts: string[] = [];
                    if (risks.length) parts.push(`${risks.length} tolerance risk${risks.length > 1 ? "s" : ""}`);
